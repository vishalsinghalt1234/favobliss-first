...
import {
  getHotProductsBanner,
  getHotProductsSection,
} from "@/data/functions/hot-products";
import { unstable_cache } from "next/cache";

const STORE_ID = process.env.NEXT_PUBLIC_STORE_ID || "684315296fa373b59468f387";

const HOT_PRODUCT_TAG = `hot-products-${STORE_ID}`;
 ...
 */
export const HotProductsSection = unstable_cache(
  async () => {
    console.log(
      `[CACHE MISS] Fetching hot products section for store: ${STORE_ID}`,
    );
    return await getHotProductsSection(STORE_ID);
  },
  [HOT_PRODUCTS_SECTION_KEY],
  {
    tags: [HOT_PRODUCT_TAG],
  },
);
 ...
 */
export const HotProductsBanner = unstable_cache(
  async () => {
    console.log(
      `[CACHE MISS] Fetching hot products banner for store: ${STORE_ID}`,
    );
    return await getHotProductsBanner(STORE_ID);
  },
  [HOT_PRODUCTS_BANNER_KEY],
  {
    tags: [HOT_PRODUCT_TAG],
  },
);
