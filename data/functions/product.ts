...
export async function productsList(
  ...
  const parsedPincode =
    typeof query.pincode === "string"
      ? parseInt(query.pincode, 10)
      : query.pincode;
  ...
  if (selectFields?.includes("variants.slug")) {
    ...
    const slugs = products.flatMap((p) =>
  ...
  const processedProducts = products.map((product: any) => {
    ...
    const sortedImages = (firstVariant?.images || []).sort((a: any, b: any) => {
      const aTime = new Date(a.createdAt).getTime();
      const bTime = new Date(b.createdAt).getTime();
      if (aTime !== bTime) return aTime - bTime;
      return a.id.localeCompare(b.id);
    ...
    const averageRating =
      numberOfRatings > 0
        ? Number(
            (ratings.reduce((a, b) => a + b, 0) / numberOfRatings).toFixed(2),
          )
        : 0;
    ...
    let selectedPrice = firstVariant?.variantPrices.find(
      (vp: any) =>
        vp.locationGroupId === resolvedLocationGroupId && vp.price > 0,
    );

    if (!selectedPrice && firstVariant?.variantPrices) {
      selectedPrice = firstVariant.variantPrices.find(
        (vp: any) => vp.price > 0,
      );
    }
...
export async function productBySlug(
  ...
  if (product.storeId !== storeId || product.isArchieved)
    throw new Error("Product not found");
  ...
  const avgRating =
    numRatings > 0
      ? ratings.reduce((a: number, b: number) => a + b, 0) / numRatings
      : 0;
  ...
  if (includeRelated && product.categoryId) {
    relatedProducts = await db.product
      .findMany({
        include: productBaseInclude,
        where: {
